        ...
        with open(filepath, "r", newline="", encoding="utf-8") as csvfile:
            ...
            for row in reader:
                ...
                try:
                    ...
                    logger.warning(
                        f"Invalid relevance value for {doc_id}: {row['relevance']}, using 0.0"
                    )
                    relevance = 0.0
        ...
        with open(filepath, "r") as f:
            for line in f:
                ...
                try:
                    ...
                    logger.warning(
                        f"Invalid relevance value for {doc_id}: {parts[3]}, using 0.0"
                    )
                    relevance = 0.0
...
def map_query_ids_to_queries(
    judgments: Dict[str, Dict[str, float]], mapping: Dict[str, str]
...
def cmd_create_experiment(args):
    ...
    if args.queries_csv:
        ...
        if isinstance(data, list):
            ...
            logger.error(
                f"Invalid JSON format in {args.queries_json}: expected a list of query strings"
            )
            return 1
    ...
    if args.judgments_csv:
        ...
        logger.info(
            f"Loaded judgments for {len(judgments)} queries from {args.judgments_csv}"
        )
        ...
        logger.info(
            f"Loaded judgments for {len(judgments)} queries from {args.judgments_trec}"
        )
    elif args.judgments_json:
        judgments = load_json(args.judgments_json)
        if not isinstance(judgments, dict):
            logger.error(
                f"Invalid JSON format in {args.judgments_json}: expected a dictionary mapping queries to relevance judgments"
            )
            return 1
        logger.info(
            f"Loaded judgments for {len(judgments)} queries from {args.judgments_json}"
        )
    ...
    if queries and not all(q in judgments for q in queries):
        ...
        logger.warning(
            f"Missing judgments for {len(missing)}/{len(queries)} queries: {missing[:5]}..."
        )
...
def cmd_list_iterations(args):
    ...
    try:
        ...
        print(
            f"Found {len(iterations)} iterations for experiment {args.experiment_id}:"
        )
...
def cmd_branch_experiment(args):
    ...
    try:
        ...
        if new_id:
            print(
                f"Created branch of experiment {args.experiment_id} with ID: {new_id}"
            )
            return 0
        else:
...
def main():
    ...
    create_parser.add_argument(
        "--id", help="Experiment ID (will be generated if not provided)"
    )
    create_parser.add_argument("--name", help="Human-readable name for the experiment")
    create_parser.add_argument("--description", help="Description of the experiment")
    create_parser.add_argument(
        "--corpus", required=True, help="Solr collection or core name"
    )
    create_parser.add_argument(
        "--metric", default="ndcg", help="Primary evaluation metric (default: ndcg)"
    )
    create_parser.add_argument(
        "--secondary-metrics",
        nargs="+",
    ...
    create_parser.add_argument(
        "--queries-csv", help="CSV file with queries (must have a 'query' column)"
    )
    create_parser.add_argument(
        "--queries-json", help="JSON file with an array of query strings"
    )
    create_parser.add_argument(
        "--judgments-csv",
    ...
    create_parser.add_argument(
        "--judgments-trec", help="TREC format file with relevance judgments"
    )
    create_parser.add_argument(
        "--query-id-mapping",
    ...
    create_parser.add_argument(
        "--judgments-json", help="JSON file with relevance judgments"
    )
    create_parser.add_argument(
        "--output", help="Output file for experiment details (JSON)"
    )
    create_parser.set_defaults(func=cmd_create_experiment)

    # run-iteration command
    run_parser = subparsers.add_parser(
        "run-iteration", help="Run an experiment iteration"
    )
    run_parser.add_argument(
        "--experiment-id", required=True, help="ID of the experiment to run"
    )
    run_parser.add_argument(
        "--id", help="Iteration ID (will be generated if not provided)"
    )
    run_parser.add_argument("--description", help="Description of the iteration")
    run_parser.add_argument(
        "--config", help="JSON file with query configuration parameters"
    )
    run_parser.add_argument(
        "--query-parser",
        default="edismax",
    ...
    run_parser.add_argument(
        "--qf", help="Query fields parameter (e.g., 'title^2.0 body^1.0')"
    )
    run_parser.add_argument("--pf", help="Phrase fields parameter")
    run_parser.add_argument("--mm", help="Minimum match parameter")
    run_parser.add_argument("--bq", help="Boost query parameter")
    run_parser.add_argument("--bf", help="Boost function parameter")
    run_parser.add_argument("--boost", help="Boost parameter")
    run_parser.add_argument("--output", help="Output file for iteration results (JSON)")
    run_parser.set_defaults(func=cmd_run_iteration)

    # compare-iterations command
    compare_parser = subparsers.add_parser(
        "compare-iterations", help="Compare two iterations of an experiment"
    )
    compare_parser.add_argument(
        "--experiment-id", required=True, help="ID of the experiment"
    )
    compare_parser.add_argument(
        "--iteration1", required=True, help="ID of the first iteration to compare"
    )
    compare_parser.add_argument(
        "--iteration2", required=True, help="ID of the second iteration to compare"
    )
    compare_parser.add_argument(
        "--output", help="Output file for comparison results (JSON)"
    )
    compare_parser.set_defaults(func=cmd_compare_iterations)

    # list-experiments command
    list_exp_parser = subparsers.add_parser(
        "list-experiments", help="List all experiments"
    )
    list_exp_parser.set_defaults(func=cmd_list_experiments)

    # list-iterations command
    list_iter_parser = subparsers.add_parser(
        "list-iterations", help="List iterations for an experiment"
    )
    ...
    export_parser = subparsers.add_parser(
        "export-experiment", help="Export an experiment to a JSON file"
    )
    export_parser.add_argument(
        "--experiment-id", required=True, help="ID of the experiment to export"
    )
    ...
    import_parser = subparsers.add_parser(
        "import-experiment", help="Import an experiment from a JSON file"
    )
    ...
    tag_parser = subparsers.add_parser(
        "tag-iteration", help="Tag an iteration with a user-friendly name"
    )
    tag_parser.add_argument(
        "--experiment-id", required=True, help="ID of the experiment"
    )
    tag_parser.add_argument(
        "--iteration-id", required=True, help="ID of the iteration to tag"
    )
    tag_parser.add_argument(
        "--tag", required=True, help="Tag to apply to the iteration"
    )
    tag_parser.set_defaults(func=cmd_tag_iteration)

    # branch-experiment command
    branch_parser = subparsers.add_parser(
        "branch-experiment", help="Create a branch of an existing experiment"
    )
    branch_parser.add_argument(
        "--experiment-id", required=True, help="ID of the experiment to branch"
    )
    branch_parser.add_argument(
        "--new-id", help="ID for the new experiment (will be generated if not provided)"
    )
    branch_parser.add_argument("--name", help="Name for the new experiment")
