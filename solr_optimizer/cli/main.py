...
def main() -> None:
    ...
    if hasattr(args, "judgments") and args.judgments is not None:
        ...
        judgments: Dict[str, Dict[str, float]]
        try:
            judgments = load_judgments_from_csv(args.judgments)
        except ValueError:
            judgments = load_judgments_from_trec(args.judgments)
    ...
    experiment_config = ExperimentConfig(
        experiment_id=None,  # Will be generated by setup_experiment
        corpus="default_corpus",  # This should be configurable via CLI
        queries=load_queries_from_csv(args.queries) if hasattr(args, "queries") and args.queries else {},
        judgments=judgments if hasattr(args, "judgments") and args.judgments else {},
        primary_metric="ndcg",  # These should be configurable via CLI
    ...
    logger.info(f"Experiment setup complete: {experiment_id}")
    
    # Execute the appropriate command
    if args.command == "run":
        # Run a single iteration with default query config
        from solr_optimizer.models.query_config import QueryConfig
        default_config = QueryConfig(
            query_parser="standard",
            additional_params={"q": "*:*"}
        )
        result = experiment_manager.run_iteration(experiment_id, default_config)
        logger.info(f"Run completed. Metrics: {result.metric_results}")
    elif args.command == "optimize":
        # Simple optimization loop - this should be replaced with actual optimization logic
        from solr_optimizer.models.query_config import QueryConfig
        for i in range(args.iterations):
            # This is a placeholder - actual optimization would generate better configs
            query_config = QueryConfig(
                query_parser="standard",
                additional_params={"q": "*:*"}
            )
            result = experiment_manager.run_iteration(experiment_id, query_config)
            logger.info(f"Iteration {i+1} completed. Metrics: {result.metric_results}")
