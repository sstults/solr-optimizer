...
class FileBasedLoggingAgent(LoggingAgent):
    ...
    def branch_experiment(
        ...
        new_id = new_experiment_id or f"{source_experiment_id}-branch-{datetime.datetime.now().strftime('%Y%m%d%H%M%S')}"
        branched_experiment = ExperimentConfig(
            **{**source_experiment.dict(), "experiment_id": new_id, "name": name or f"Branch of {source_experiment_id}"}
        )
